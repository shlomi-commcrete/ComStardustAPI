...
interface MessagesDao {
    ...
    @Query("SELECT * FROM messages_table WHERE TRIM(LOWER(chatId)) = TRIM(LOWER(:chatId)) AND isArchived = 0 ORDER BY epochTimeMs ASC")
    ...
    @Query("DELETE FROM messages_table WHERE TRIM(LOWER(chatId)) = TRIM(LOWER(:chatId))")
    fun clearChat(chatId : String)

    @Query("DELETE FROM messages_table WHERE TRIM(LOWER(chatId)) = TRIM(LOWER(:chatId)) AND epochTimeMs >= :startTimestamp AND epochTimeMs < :endTimestamp")
    suspend fun clearChatInRange(chatId : String,
                         startTimestamp: Long,
                         endTimestamp: Long)

    @Query("UPDATE messages_table SET seen=:isSeen WHERE TRIM(LOWER(chatId)) = TRIM(LOWER(:chatId))")
    ...
    @Query("UPDATE messages_table SET seen=2 WHERE TRIM(LOWER(chatId)) = TRIM(LOWER(:chatId)) AND id_number=:messageNumber")
